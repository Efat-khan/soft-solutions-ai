  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-6 pt-20 overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
        ...
        />
        <motion.div
          animate={{ scale: [1, 1.3, 1], opacity: [0.05, 0.15, 0.05], x: [0, -40, 0], y: [0, -60, 0] }}
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="z-10 max-w-6xl">
        {/* Badge */}
        <motion.div
        ...
        >
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-intelligence-blue opacity-75" />
            <span className="relative inline-flex rounded-full h-2 w-2 bg-intelligence-blue" />
          </span>
          {h.badge}
        </motion.div>
        ...
        >
          {h.sub1}{" "}
          <span className="text-white/80">{h.commerce}</span>,{" "}
          <span className="text-white/80">{h.education}</span>,{" "}
          {t.nav.services === "সেবাসমূহ" ? "এবং" : "and"}{" "}
          <span className="text-white/80">{h.automation}</span>.{" "}
          {h.sub2}
        </motion.p>
        ...
        <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-center justify-center gap-6">
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="group px-10 py-5 bg-intelligence-blue rounded-2xl font-bold shadow-2xl glow-blue transition-all flex items-center gap-3"
            style={{ color: "white" }}
          >
            {h.btn1}
      ...
      >
        <div className="flex items-center gap-2 mb-3 border-b border-white/10 pb-2">
          ...
          <span className="font-mono text-[10px] tracking-widest text-white/30 uppercase">{h.terminal}</span>
        </div>
